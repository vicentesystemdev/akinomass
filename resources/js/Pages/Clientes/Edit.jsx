...
export default function Edit(props) {
    ...
    const { data, setData, put, processing, errors, clearErrors } = useForm({
        ...
        estado_cli: cliente.estado_cli ?? estados[0] ?? 'activo',
    ...
    const hasChanges = useMemo(() => {
        return (
            String(data.nombre_cli ?? '') !== String(cliente.nombre_cli ?? '') ||
            String(data.telefono_cli ?? '') !== String(cliente.telefono_cli ?? '') ||
            String(data.correo_cli ?? '') !== String(cliente.correo_cli ?? '') ||
            String(data.direccion_cli ?? '') !== String(cliente.direccion_cli ?? '') ||
            String(data.documento_cli ?? '') !== String(cliente.documento_cli ?? '') ||
            String(data.observacion_cli ?? '') !== String(cliente.observacion_cli ?? '') ||
            String(data.estado_cli ?? '') !== String(cliente.estado_cli ?? '') ||
            String(data.cod_canal_venta ?? '') !== String(cliente.cod_canal_venta ?? '') ||
            String(data.cod_tipo_flujo_comercial ?? '') !==
                String(cliente.cod_tipo_flujo_comercial ?? '')
        );
    }, [data, cliente]);
    ...
    const submit = (e) => {
        ...
        put(route('clientes.update', cliente.cod_cliente), {
            preserveScroll: true,
        });
    };
...
function obtenerLecturaComercial(data) {
    if (!data.nombre_cli || !data.telefono_cli) {
        return 'Ficha incompleta. Registra nombre y teléfono para mantener seguimiento comercial.';
    }

    if (!data.cod_canal_venta && !data.cod_tipo_flujo_comercial) {
        return 'Cliente editable, pero falta canal y flujo. Completar estos datos mejora análisis de ventas.';
    }
    ...
    if (normalizarEstado(data.estado_cli).includes('inactivo')) {
        return 'Cliente inactivo. Conviene revisar si corresponde seguimiento, reactivación o conservación histórica.';
    }
