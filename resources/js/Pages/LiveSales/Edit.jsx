...
export default function Edit({ sesion, canales, estados }) {
    const form = useForm({
        titulo_ses: sesion?.titulo_ses ?? '',
        fecha_inicio_ses: sesion?.fecha_inicio_ses?.slice(0, 16) ?? '',
        fecha_fin_ses: sesion?.fecha_fin_ses?.slice(0, 16) ?? '',
        estado_ses: sesion?.estado_ses ?? (estados?.[0] || 'programada'),
        cod_canal_venta: sesion?.cod_canal_venta ?? '',
        resumen_ses: sesion?.resumen_ses ?? '',
    });

    const submit = () => {
        form.put(route('live-sales.update', sesion.cod_sesion_live));
    };
    ...
    return (
        <AuthenticatedLayout
            header={
                <PageHeader
                    title="Editar Sesión"
                    subtitle={`Modifique los datos de "${sesion.titulo_ses}"`}
                    breadcrumbs={[
                        { label: 'Dashboard', href: route('dashboard') },
                        { label: 'Live Sales', href: route('live-sales.index') },
                        { label: sesion.titulo_ses, href: route('live-sales.show', sesion.cod_sesion_live) },
                        { label: 'Editar' },
                    ]}
                />
            }
        >
            <Head title="Editar Sesión Live" />

            <div className="max-w-2xl mx-auto">
                <FormCard
                    title="Datos de la Sesión"
                    subtitle="Modifique la información de la sesión"
                ...
                >
                    ...
                    <FormCard.Actions>
                        <Link href={route('live-sales.show', sesion.cod_sesion_live)}>
