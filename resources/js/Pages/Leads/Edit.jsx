...
export default function Edit(props) {
  ...
  const { data, setData, put, processing, errors, clearErrors } = useForm({
    ...
    estado_lea: lead.estado_lea ?? estados[0] ?? 'nuevo',
    fecha_seguimiento_lea: normalizarFecha(lead.fecha_seguimiento_lea),
  ...
  const hasChanges = useMemo(() => {
    return (
      String(data.nombre_lea ?? '') !== String(lead.nombre_lea ?? '') ||
      String(data.alias_lea ?? '') !== String(lead.alias_lea ?? '') ||
      String(data.telefono_lea ?? '') !== String(lead.telefono_lea ?? '') ||
      String(data.correo_lea ?? '') !== String(lead.correo_lea ?? '') ||
      String(data.producto_interes_lea ?? '') !==
      String(lead.producto_interes_lea ?? '') ||
      String(data.observacion_lea ?? '') !==
      String(lead.observacion_lea ?? '') ||
      String(data.estado_lea ?? '') !== String(lead.estado_lea ?? '') ||
      String(data.fecha_seguimiento_lea ?? '') !==
      String(normalizarFecha(lead.fecha_seguimiento_lea) ?? '') ||
      String(data.cod_canal_venta ?? '') !==
      String(lead.cod_canal_venta ?? '') ||
      String(data.cod_tipo_flujo_comercial ?? '') !==
      String(lead.cod_tipo_flujo_comercial ?? '') ||
      String(data.cod_usuario_responsable ?? '') !==
      String(lead.cod_usuario_responsable ?? '')
    );
  }, [data, lead]);
  ...
  const submit = (e) => {
    ...
    put(route('leads.update', lead.cod_lead), {
      preserveScroll: true,
    });
  };
        ...
        {wasValidated && !validationStatus.isValid && (
          <section className="rounded-3xl border border-red-200 bg-red-50 p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-red-600">
              Hay datos obligatorios o información comercial con formato
              inválido. Corrige los campos marcados antes de actualizar.
            </p>
          </section>
        )}
        ...
        {leadConvertido && (
          <section className="rounded-3xl border border-green-200 bg-green-50 p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm leading-6 text-green-700">
              Este lead ya está marcado como convertido. Desde la gestión
              comercial, el seguimiento principal debería continuar desde el
              módulo de clientes.
            </p>
          </section>
        )}
        ...
        <form onSubmit={submit} noValidate className="grid gap-6 xl:grid-cols-3">
          <section className="space-y-6 xl:col-span-2">
            <FormCard
              ...
              description="Identifica al prospecto y actualiza referencias útiles para contacto o reconocimiento."
            >
            ...
            <FormCard
              title="Contacto"
              description="Actualiza los datos necesarios para continuar el proceso comercial."
            >
              <div className="grid gap-5 md:grid-cols-2">
                <FormField
                  label="Teléfono"
            ...
            <FormCard
              ...
              description="Mantén claro qué producto, línea o promoción motivó el contacto del lead."
            >
              <FormField
                label="Producto de interés"
                error={allErrors.producto_interes_lea}
                helper="Entre 3 y 180 caracteres. Ayuda a preparar una respuesta comercial más precisa."
                required
              >
                <input
                  type="text"
            ...
            <FormCard
              ...
              description="Gestiona el estado comercial, canal, flujo, responsable y próxima fecha de contacto."
            >
              <div className="grid gap-5 md:grid-cols-2">
                <FormField
                ...
                <FormField
                  ...
                  helper="Recomendado. Permite saber qué medio generó el lead."
                >
                  <select
                ...
                <FormField
                  ...
                  helper="Recomendado. Ayuda a definir estrategia de atención."
                >
                  <select
...
function obtenerLecturaComercial(data) {
  ...
  if (normalizarEstado(data.estado_lea).includes('convertido')) {
    return 'Lead convertido. El seguimiento principal debería continuar como cliente.';
  }

  if (!data.cod_usuario_responsable) {
    return 'Lead válido, pero sin responsable. Conviene asignarlo para evitar pérdida de seguimiento.';
  }
