...
class RegistrarEntradaInventarioAction
{
    ...
    {
        $inventario = $this->service->registrarMovimiento($data['cod_producto'], TipoMovimientoInventarioEnum::ENTRADA, $data['cantidad_mov'], $data['motivo_mov'], $data['observacion_mov'] ?? null, $codUsuario);

        $contexto = RegistrarAuditoriaData::fromRequest(request());
        $this->auditoriaService->registrarInsercion($contexto, 'Inventario', 'movimientos_inventario', (string) $data['cod_producto'], submodulo: 'Entrada');
